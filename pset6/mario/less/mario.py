...
'''
# we also draw some pyramid using recursion!
def letsDrawPyramid(height, width):
    if height == 0:  # check if height is 0 and nothing to draw
        return
    # and now we draw our pyramid calling recursion function with same width but height - 1
    letsDrawPyramid(height - 1, width)
    print(" " * (width - height), end='')  # here we print as much `spaces` as needed
    print("#" * height)
'''
